---
title: "Basic plot generator for Science-Fiction stories and the like (Shiny app)"
output:
  html_document
runtime: shiny
---
...
This R Markdown document is made interactive using Shiny technology. 
...
with Deception
with poverty=to win an inheritance=with the cost of wealth=in a vicious competition for great wealth=in a vicious competition for a paltry sum=to keep rightfully owned property=to keep ill-gotten gains| (Money)
with Authority
to Make Amends
with a Conspiracy
to Rescue someone
to develop a personal sense of=with a dissolving=against a mistaken=to conceal their true| Identity
with |Illicit=Sick=True| Love
with |having committed a=the urge to commit a=the desire to commit a=the necessity to commit a=recovering from a life of| Crime
with Suspicion
with Suicide
and Searches
with |Dishonor=Honor")
...
the point of view is the NPC's perspective
there's a major role-reversal halfway through
we have an uncommon partner: we must work with someone |we'd rather not=we want to kill=who wants to kill us
...
our predecessors have already failed -- we find |them locked up=them dying=them dead=them gone native=them completely insane=their possessions
...
inputPanel(
    actionButton("run", label = "Give me a plot!")
)
...
renderUI({
    ...
    return(list(
        ...
        h3("Hero"),
        HTML('This story is about a <b>', randtxt(table_hero), '</b> (<b>', randtxt(table_sex), '</b>) who struggles <b>', randtxt(table_theme), '</b> but does it because of <b>', randtxt(table_motive), '</b>.'),
        h3("Main plot"),
        HTML(randtxt(table_mainplot)),
        h3("Twist"),
        HTML('In this story, <b>', randtxt(table_twist), '</b>.')
    ))
})
```
